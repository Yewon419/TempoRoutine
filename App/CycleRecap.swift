...
// 나의 템포 데이터의 재조합일 뿐 새 계산 축은 없다. 노출 = 나의 템포 탭 표제 아래 1회(닫으면 그 주기
// 종료), 발행 순간이 「한 바퀴」(firstCycle) 업적의 훅이다. 오늘 탭 상단(초판)은 85차에 철수 — 「오늘 할 일」
// 사이에 읽을거리가 끼어 「없애자」가 됐다. 겨울 전환 인사는 SeasonTurnCard(오늘 탭)가 맡는다.
//
