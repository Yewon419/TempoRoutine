...
//   (설치 직후 가짜 전환 방지).
// - 겨울 전환도 이 카드가 인사한다(2026-09-12). 종전엔 리캡(A3)이 오늘 탭에서 겨울을 맡아 건너뛰었지만
//   리캡이 나의 템포 탭으로 옮겨가(85차 오늘 탭 철수 → 90차 이관) 두 카드가 더는 한 화면에서 겹치지 않는다.
...
enum SeasonTurnStore {
    ...
    static func greeting(for phase: CyclePhase) -> String {
        ...
        case .menstrual:  Loc.str("겨울이 시작됐어요. 조금은 쉬어가도 좋아요.")
        }
    }
}
