...
- **ci.yml 같은 공용 파일에 `sed` 패턴 치환 금지 — 매치 줄을 먼저 `grep -n`으로 전수 확인하거나 Edit로
  줄 단위 수정**(2026-09-06 실측: 온보딩 찰칵 스텝만 고치려던 `-appLanguage ko$` 치환이 기존 4탭 스텝
  4줄까지 바꿔 범위 밖 커밋이 나갔고 원복 커밋을 한 번 더 태웠다).
- **하단바 외관은 찰칵(시뮬) 컷으로 판정하지 않는다**(2026-09-11 실측). 날씨 테마는 실기기 베타 컷에서
  다크 플레이트가 제대로 나오는데 같은 코드의 시뮬 컷은 밝은 유리로 나왔다 — `toolbarBackground(for: .tabBar)`
  강제의 렌더가 시뮬과 실기기에서 갈린다. 탭바 배경·틴트 수정은 TestFlight 실기기 확인 전까지 "됐다"가 아니다.
  (지면·표제·카드·조판은 찰칵으로 계속 본다.)
- **찰칵 ko 컷은 `-appLanguage ko`만으론 반쪽이다** — 앱 `Loc` 층만 바뀌고 SwiftUI `Text` 리터럴(xcstrings)은
  시뮬레이터 시스템 로케일(en)을 따른다(2026-09-06 실측: 표제·본문 영어, 버튼만 한글). `-AppleLanguages (ko)`를
  같이 줘야 번들 로컬라이제이션까지 ko. 기존 4탭 스텝은 대표님 결정 대기.
- **`scaledToFill` 이미지는 `clipShape`/`clipped`로 그림만 잘릴 뿐 히트 영역은 넘친 채 남는다**(2026-09-26
  iPad 실측: 테마 탭 티켓 미니 카드의 정사각 사진이 넓은 띠를 채우며 ±260pt 넘쳐 위 카드의 미리보기·적용·
  포인트컬러를 덮었다 — 보이는 건 멀쩡, 폭이 좁은 아이폰에선 티가 안 남). 버튼 근처의 장식 지면은
