...
# (의학 단계명 4종은 2026-08-21 소스에서 삭제 — SeasonMeta.phaseName 필드 자체를 없앴다.
#  개정 M-1c가 표면 금지인데 필드가 있으니 갖다 쓰는 사고가 났다. 이제 목록에 둘 이유가 없다.)
IGNORED_LITERALS: frozenset[str] = frozenset({
    ...
    "대체공휴일(",
    # TempoCore 안 합성 2건 — 앱이 대신 합성해 번역한다(Loc.holidayName · symptomAnchorTitle).
    # 순수 모듈이라 Loc이 닿지 않는다. 근본 해결은 반환 타입 구조화(로컬라이제이션.md §5).
    "대체공휴일(\(entry.name))",
    "「\(choice.label)」, 평소와 비교해서 답해주세요.",
})
